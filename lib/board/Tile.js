(function() {
	L7.Tile = function(config) {
		...
		this._inhabitants = _.clone(config.inhabitants || []);
		this.position = L7.p(this.x, this.y);
	};

	L7.Tile.prototype = {
		at: function(index) {
			return this._inhabitants[index];
		},
		each: function(operation) {
			this._inhabitants.forEach(operation);
		},
		add: function(inhabitant) {
			this._inhabitants.push(inhabitant);
		},
		remove: function(inhabitant) {
			this._inhabitants.remove(inhabitant);
		},
		has: function(team) {
			return this._inhabitants.some(function(inhabitant) {
	...
	Object.defineProperty(L7.Tile.prototype, 'count', {
		get: function() {
			return this._inhabitants.length;
		},
		enumerable: true
	});
})();
