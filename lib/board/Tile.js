(function() {
	...
	L7.Tile.prototype = {
		...
		render: function(delta, context, tileSize) {
			var args = _.toArray(arguments);
			if (this.color) {
				this._renderColor(context, this.color, tileSize);
			}

			this.each(function(inhabitant) {
				//inhabitant.render.apply(inhabitant, args);
				if(inhabitant.color) {
					this._renderColor(context, inhabitant.color, tileSize);
				}
			}, this);
		},
		_renderColor: function(context, color, size) {
			context.save();
