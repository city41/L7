(function() {
	...
	L7.Board.prototype = {
		...
		moveActor: function(options) {
			var pieceSets = options.actor.pieceSets || [[options.actor.pieces]];

			pieceSets.forEach(function(frames) {
				frames.forEach(function(pieces) {
					pieces.forEach(function(piece) {
						this._movePiece(piece, options.delta || options.to.delta(options.from), pieces === options.actor.pieces);
					},
					this);
				},
				this);
