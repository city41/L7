(function() {
	...
	L7.Board.prototype = {
		...
		setTileColors: function(gl, anchorXpx, anchorYpx) {
			var bw = this.borderWidth,
			ts = this.tileSize,
			...
			colorData,
			blankColor = [0, 0, 0, 0],
			standardBorderColor = this.borderFill ? L7.Color.toArray(this.borderFill) : blankColor;
				...
				if (y >= 0) {
					...
					for (x = seedx; x < xl; ++x) {
						if (x >= 0) {
							...
							var borderColor = color[3] ? standardBorderColor : blankColor;
							for(var b = 0; b < 24; ++b) {
								colorData.push(borderColor[0] / 255);
								colorData.push(borderColor[1] / 255);
								colorData.push(borderColor[2] / 255);
								colorData.push(borderColor[3]);
							}
						}
					}
					// bufferSubData here
		...
		rendergl: function(delta, gl, anchorXpx, anchorYpx, timestamp) {
			...
			this.setTileColors(gl, anchorXpx, anchorYpx);
