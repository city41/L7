(function() {
	...
	L7.Board.prototype = {
		...
		render: function(delta, context, anchorX, anchorY, timestamp) {
			...
			ts = this.tileSize,
			seedy = anchorY,
			y, yl = this._rows.length,
			seedx = anchorX,
			x, xl = this._rows[0].length,
			tile,
			color,
			lastColor,
			row;
						...
						if (x >= 0) {
							...
							if (scale !== 0 && color) {
								if (scale <= 1) {
									...
									c.fillRect((x - seedx) * (ts + bw) + bw + offset, (y - seedy) * (ts + bw) + bw + offset, size, size);
								} else {
									scaledOut.push(tile);
								}
							}
			...
			for (var i = 0; i < scaledOut.length; ++i) {
				...
				if (color) {
					...
					var size = Math.round(ts * scale);
					var offset = ts / 2 - size / 2;
					c.fillRect((tile.x - seedx) * (ts + bw) + bw + offset, (tile.y - seedy) * (ts + bw) + bw + offset, size, size);
				}
			}
		}
	};
