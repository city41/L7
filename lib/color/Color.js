(function() {
	function _composite(under, over) {
		var alphaO = over[3] / 255;
		var alphaU = under[3] / 255;
	...
	function _hexToArray(hex) {
		...
		result = [parseInt(oxr, 16), parseInt(oxg, 16), parseInt(oxb, 16), 255];
	...
	L7.Color = {
		...
		composite: function(colorVarArgs) {
			var output = this.toArray(arguments[0]);
