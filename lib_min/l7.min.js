...
[],b=this.weights&&_.isNumber(this.weights[0])?this.weights[0]:1,c=this.weights&&_.isNumber(this.weights[1])?this.weights[1]:1,d=this.weights&&_.isNumber(this.weights[2])?this.weights[2]:1,e=this.alpha||1,f=this.noise||1,g=0,h,o,j;256>g;g++)h=~~(128+128*Math.sin(Math.PI*g/(32/f))),o=~~(128+128*Math.sin(Math.PI*g/(64/f))),j=~~(128+128*Math.sin(Math.PI*g/(128/f))),h=Math.min(255,h*b),o=Math.min(255,o*c),j=Math.min(255,j*d),a[g]=[h,o,j,e];return a},_determineDim:function(a,b){var c=Math.min.apply(null,
...
a.delta||a.to.delta(a.from))},this);a.actor.onOutOfBounds&&this.isOutOfBounds(a.actor)&&a.actor.onOutOfBounds.call(a.actor)},update:function(a,b){this.actors.forEach(function(c){c.update(a,b)});this.freeActors.forEach(function(c){c.update(a,b)});this.daemons.forEach(function(c){c.update(a,b,this)},this);this._hitManager.detectHits(this.tiles)},render:function(a,b,c,d){this.angle&&(b.save(),b.rotate(this.angle));var c=c+(this.offsetX||0),d=d+(this.offsetY||0),e=this.borderWidth,a=this.tileSize,f=(0>
d?Math.ceil:Math.floor)(d/(a+e)),g=-d%(a+e),h,o=Math.min(this._rows.length,Math.ceil((d+b.canvas.height)/(a+e))),j=(0>c?Math.ceil:Math.floor)(c/(a+e)),k=-c%(a+e),l,r=Math.min(this._rows[0].length,Math.ceil((c+b.canvas.width)/(a+e))),n,i,q,p=[];for(h=f;h<o;++h)if(0<=h){q=this._rows[h];for(l=j;l<r;++l)if(0<=l&&(n=q[l],i=n.getColor()))m=n.getScale(),_.isNumber(m)||(m=1),1!==m&&(p.push(n),i=n.getColor(!0),m=1),i&&(this.borderFill&&(b.fillStyle=this.borderFill,b.fillRect((l-j)*(a+e)+k,(h-f)*(a+e)+g,a+
2*e,e),b.fillRect((l-j)*(a+e)+k,(h-f)*(a+e)+g+a+e,a+2*e,e),b.fillRect((l-j)*(a+e)+k,(h-f)*(a+e)+g,e,a+2*e),b.fillRect((l-j)*(a+e)+k+a+e,(h-f)*(a+e)+g,e,a+2*e)),b.fillStyle=i,i=Math.round(a*m),m=a/2-i/2,b.fillRect((l-j)*(a+e)+e+m+k,(h-f)*(a+e)+e+m+g,i,i))}p.sort(function(a,c){return a.scale-c.scale});for(h=0;h<p.length;++h){n=p[h];var m=n.getScale();if(i=n.getColor())b.fillStyle=i,i=Math.round(a*m),m=a/2-i/2,b.fillRect((n.x-j)*(a+e)+e+m+k,(n.y-f)*(a+e)+e+m+g,i,i)}for(h=0;h<this.freeActors.length;++h)if(e=
this.freeActors[h],i=L7.Color.toCssString(e.color))b.fillStyle=i,b.fillRect(e.position.x-c,e.position.y-d,a,a);this.angle&&b.restore()}}})();
...
b),this.opaque&&(b[3]=1),L7.Color.toCssString(b)},getColorOld:function(){var a;if(0===this.inhabitants.length||!this.inhabitants[this.inhabitants.length-1].color)a=this.color;else if(a=this.inhabitants[this.inhabitants.length-1],L7.Color.isOpaque(a.color)||!this.color)a=a.color;else{var b=this.color.slice(0);a=L7.Color.composite(b,a.color)}this.overlayColor&&(a=!a||L7.Color.isOpaque(this.overlayColor)?this.overlayColor:L7.Color.composite(a.slice(0),this.overlayColor));if(a)return L7.Color.toCssString(a)},
...
(function(){var a=1E3*(1/60);window.requestAnimationFrame=window.requestAnimationFrame||window.mozRequestAnimationFrame||window.webkitRequestAnimationFrame||window.msRequestAnimationFrame||window.oRequestAnimationFrame||function(a){window.setTimeout(function(){a(Date.now())},1E3/60)};L7.Game=function(a){a="function"!==typeof a.render&&"function"!==typeof a.update?a:{width:a.width*(a.tileSize+a.borderWidth)+a.borderWidth,height:a.height*(a.tileSize+a.borderWidth)+a.borderWidth,board:a};_.extend(this,
a);_.bindAll(this,"_doFrame");this.viewport=new L7.Viewport(this);this.board&&(this.board.viewport=this.viewport);this.container=this.container||document.body;this.canvas=this._createCanvas();L7.Keys.init(this.canvas);L7.Mouse.init(this.canvas);this.delays=[];var c=this;L7.global.addEventListener("blur",function(){delete c._lastTimestamp})};L7.Game.prototype={_createCanvas:function(){var a=document.createElement("canvas");a.width=this.viewport.width;a.height=this.viewport.height;a.style.imageRendering=
"-webkit-optimize-contrast";this.container.appendChild(a);return a},go:function(){delete this._lastTimestamp;this._doFrame(Date.now())},after:function(a,c){this.delays.add({remaining:a,handler:c})},replaceBoard:function(a){this.board&&this.board.destroy&&this.board.destroy();this.board=a;this.board.viewport=this.viewport;this.viewport.reset()},_updateDelays:function(a){var c=[];this.delays.forEach(function(d){d.remaining-=a;0>=d.remaining&&(d.handler.call(d.scope,this),c.push(d))},this);c.forEach(function(a){this.delays.remove(a)},
this)},_pause:function(){var a=this.canvas.getContext("2d");a.save();a.fillStyle="rgba(255,255,255,0.5)";a.fillRect(0,0,this.canvas.width,this.canvas.height);a.restore()},_doFrame:function(b){var c=b-(this._lastTimestamp||b);this._lastTimestamp=b;for(var d=c;0<d;){var e=Math.min(a,d);this._updateDelays(e);this.board.update(e,b);d-=e}d=this.canvas.getContext("2d");d.clearRect(0,0,this.canvas.width,this.canvas.height);this.board.render(c,d,this.viewport.anchorX,this.viewport.anchorY,b);this.paused||
requestAnimationFrame(this._doFrame)}};Object.defineProperty(L7.Game.prototype,"paused",{get:function(){return this._paused},set:function(a){var c=this._paused;this._paused=a;c!==a&&(a?this._pause():this.go())}})})();
