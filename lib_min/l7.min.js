...
(function(){for(var a=[],b=0,c,d,e;256>b;b++)c=~~(128+128*Math.sin(Math.PI*b/32)),d=~~(128+128*Math.sin(Math.PI*b/64)),e=~~(128+128*Math.sin(Math.PI*b/128)),a[b]=[c,d,e,1];L7.Plasma=function(a){_.extend(this,a);this._paletteOffset=0;this._elapsed=this.rate;this._width=this._determineDim(this.targets,"x");this._height=this._determineDim(this.targets,"y")};L7.Plasma.prototype={update:function(a){this._elapsed+=a;this._elapsed>=this.rate&&(this._doPlasma(this._width,this._height,this.targets,this._paletteOffset),
this._elapsed-=this.rate,this._paletteOffset+=1)},_determineDim:function(a,c){var b=Math.min.apply(null,a.map(function(a){return a.position[c]}));return Math.max.apply(null,a.map(function(a){return a.position[c]}))-b+1},_doPlasma:function(c,b,d,e){for(var j=0,i;j<b;++j)for(i=0;i<c;++i)d[c*j+i].overlayColor=a[~~(0.25*(128*Math.sin(0.0625*i)+256+128*Math.sin(0.03125*j)+128+128*Math.sin(0.125*Math.sqrt((i+e-c)*(i+e-c)+(j-e-b)*(j-e-b)))+128+128*Math.sin(0.125*Math.sqrt(i*i+j*j)))+e)%256]}}})();
...
h,p=Math.min(this._rows.length,Math.ceil((d+b.canvas.height)/(a+e))),j=(0>c?Math.ceil:Math.floor)(c/(a+e)),i=-c%(a+e),l,r=Math.min(this._rows[0].length,Math.ceil((c+b.canvas.width)/(a+e))),n,k,q,o=[];for(h=f;h<p;++h)if(0<=h){q=this._rows[h];for(l=j;l<r;++l)if(0<=l&&(n=q[l],k=n.getColor()))m=n.getScale(),_.isNumber(m)||(m=1),1!==m&&(o.push(n),k=n.getColor(!0),m=1),k&&(this.borderFill&&(b.fillStyle=this.borderFill,b.fillRect((l-j)*(a+e)+i,(h-f)*(a+e)+g,a+2*e,e),b.fillRect((l-j)*(a+e)+i,(h-f)*(a+e)+
g+a+e,a+2*e,e),b.fillRect((l-j)*(a+e)+i,(h-f)*(a+e)+g,e,a+2*e),b.fillRect((l-j)*(a+e)+i+a+e,(h-f)*(a+e)+g,e,a+2*e)),b.fillStyle=k,k=Math.round(a*m),m=a/2-k/2,b.fillRect((l-j)*(a+e)+e+m+i,(h-f)*(a+e)+e+m+g,k,k))}o.sort(function(a,c){return a.scale-c.scale});for(h=0;h<o.length;++h){n=o[h];var m=n.getScale();if(k=n.getColor())b.fillStyle=k,k=Math.round(a*m),m=a/2-k/2,b.fillRect((n.x-j)*(a+e)+e+m+i,(n.y-f)*(a+e)+e+m+g,k,k)}for(h=0;h<this.freeActors.length;++h)if(e=this.freeActors[h],k=L7.Color.toCssString(e.color))b.fillStyle=
k,b.fillRect(e.position.x-c,e.position.y-d,a,a)}}})();
...
if(this.isRgbString(c)){for(var d=c.indexOf("("),e=[],c=c.substring(d+1).split(","),d=0,f=c.length;d<f;++d)e.push(parseFloat(c[d]));3===e.length&&e.push(1);return e}},fromArrayToHex:function(a,b){for(var e="#",f=b&&b.includeAlpha&&4===a.length?4:3,g=0;g<f;++g){var h=a[g].toString(16);2>h.length&&(h="0"+h);e+=h}return e.toUpperCase()},composite:function(a){for(var b=this.toArray(arguments[0]).slice(0),e=1;e<arguments.length;++e){var f=b,g=this.toArray(arguments[e]),h=g[3],p=f[3],j=1-h,i=void 0,l=void 0;
for(i=0,l=f.length-1;i<l;++i)f[i]=Math.round(g[i]*h+f[i]*p*j);f[3]=(f[3]+g[3])/2}return b},fromFloats:function(a,b,e,f){return[Math.round(255*a),Math.round(255*b),Math.round(255*e),f]}};var b={aliceblue:"#F0F8FF",antiquewhite:"#FAEBD7",aqua:"#00FFFF",aquamarine:"#7FFFD4",azure:"#F0FFFF",beige:"#F5F5DC",bisque:"#FFE4C4",black:"#000000",blanchedalmond:"#FFEBCD",blue:"#0000FF",blueviolet:"#8A2BE2",brown:"#A52A2A",burlywood:"#DEB887",cadetblue:"#5F9EA0",chartreuse:"#7FFF00",chocolate:"#D2691E",coral:"#FF7F50",
