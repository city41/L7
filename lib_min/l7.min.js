...
[],b=this.weights&&_.isNumber(this.weights[0])?this.weights[0]:1,c=this.weights&&_.isNumber(this.weights[1])?this.weights[1]:1,d=this.weights&&_.isNumber(this.weights[2])?this.weights[2]:1,e=this.alpha||1,f=this.noise||1,g=0,h,o,k;256>g;g++)h=~~(128+128*Math.sin(Math.PI*g/(32/f))),o=~~(128+128*Math.sin(Math.PI*g/(64/f))),k=~~(128+128*Math.sin(Math.PI*g/(128/f))),h=Math.min(255,h*b),o=Math.min(255,o*c),k=Math.min(255,k*d),a[g]=[h,o,k,e];return a},_determineDim:function(a,b){var c=Math.min.apply(null,
...
a.delta||a.to.delta(a.from))},this);a.actor.onOutOfBounds&&this.isOutOfBounds(a.actor)&&a.actor.onOutOfBounds.call(a.actor)},update:function(a,b){this.actors.forEach(function(c){c.update(a,b)});this.freeActors.forEach(function(c){c.update(a,b)});this.daemons.forEach(function(c){c.update(a,b,this)},this);this._hitManager.detectHits(this.tiles)},squareVertexPositionBuffer:void 0,initBuffer:function(a){this.squareVertexPositionBuffer=a.createBuffer();a.bindBuffer(a.ARRAY_BUFFER,this.squareVertexPositionBuffer);
a.bufferData(a.ARRAY_BUFFER,new Float32Array([0,1,0,0,0,0,1,1,0,1,0,0]),a.STATIC_DRAW);this.squareVertexPositionBuffer.itemSize=3;this.squareVertexPositionBuffer.numItems=4},_initGl:function(a){a.clearColor(0,0,0,1);this.pMatrix=mat4.create();this.mvMatrix=mat4.create();this.initBuffer(a);this.initShaders(a)},getShader:function(a,b){var c=document.getElementById(b);if(!c)return null;for(var d="",e=c.firstChild;e;)3==e.nodeType&&(d+=e.textContent),e=e.nextSibling;if("x-shader/x-fragment"==c.type)c=
a.createShader(a.FRAGMENT_SHADER);else if("x-shader/x-vertex"==c.type)c=a.createShader(a.VERTEX_SHADER);else return null;a.shaderSource(c,d);a.compileShader(c);return!a.getShaderParameter(c,a.COMPILE_STATUS)?(alert(a.getShaderInfoLog(c)),null):c},initShaders:function(a){var b=this.getShader(a,"shader-fs"),c=this.getShader(a,"shader-vs");this.shaderProgram=a.createProgram();a.attachShader(this.shaderProgram,c);a.attachShader(this.shaderProgram,b);a.linkProgram(this.shaderProgram);a.getProgramParameter(this.shaderProgram,
a.LINK_STATUS)||alert("Could not initialise shaders");a.useProgram(this.shaderProgram);this.shaderProgram.vertexPositionAttribute=a.getAttribLocation(this.shaderProgram,"aVertexPosition");a.enableVertexAttribArray(this.shaderProgram.vertexPositionAttribute);this.shaderProgram.colorUniform=a.getUniformLocation(this.shaderProgram,"uColor");this.shaderProgram.pMatrixUniform=a.getUniformLocation(this.shaderProgram,"uPMatrix");this.shaderProgram.mvMatrixUniform=a.getUniformLocation(this.shaderProgram,
"uMVMatrix")},renderTile:function(a,b,c,d,e){mat4.identity(this.mvMatrix);mat4.translate(this.mvMatrix,[b,c,0]);mat4.scale(this.mvMatrix,[d,d,0]);a.uniformMatrix4fv(this.shaderProgram.pMatrixUniform,!1,this.pMatrix);a.uniformMatrix4fv(this.shaderProgram.mvMatrixUniform,!1,this.mvMatrix);a.uniform4fv(this.shaderProgram.colorUniform,e);a.drawArrays(a.TRIANGLE_STRIP,0,this.squareVertexPositionBuffer.numItems)},rendergl:function(a,b,c,d){this.squareVertexPositionBuffer||this._initGl(b);b.viewport(0,0,
b.viewportWidth,b.viewportHeight);b.clear(b.COLOR_BUFFER_BIT);mat4.ortho(0,b.viewportWidth,b.viewportHeight,0,-100,100,this.pMatrix);b.bindBuffer(b.ARRAY_BUFFER,this.squareVertexPositionBuffer);b.vertexAttribPointer(this.shaderProgram.vertexPositionAttribute,this.squareVertexPositionBuffer.itemSize,b.FLOAT,!1,0,0);this.drawTiles(b,c,d,b.viewportWidth,b.viewportHeight)},drawTiles:function(a,b,c,d,e){for(var b=b+(this.offsetX||0),c=c+(this.offsetY||0),f=this.borderWidth,g=this.tileSize,h=c/(g+f)|0,
o=-c%(g+f),c=Math.min(this._rows.length,Math.ceil((c+e)/(g+f))),e=b/(g+f)|0,k=-b%(g+f),l=Math.min(this._rows[0].length,Math.ceil((b+d)/(g+f))),i,p,m,j=[],b=h;b<c;++b)if(0<=b){m=this._rows[b];for(d=e;d<l;++d)if(0<=d&&(i=m[d],p=i.getColor()))if(scale=i.getScale(),_.isNumber(scale)||(scale=1),1!==scale&&(j.push(i),p=i.getColor(!0),scale=1),p){i=Math.round(g*scale);var q=g/2-i/2;this.renderTile(a,(d-e)*(g+f)+f+q+k,(b-h)*(g+f)+q+o,i,p)}}},render:function(a,b,c,d){var c=c+(this.offsetX||0),d=d+(this.offsetY||
0),e=this.borderWidth,a=this.tileSize,f=d/(a+e)|0,g=-d%(a+e),h,o=Math.min(this._rows.length,Math.ceil((d+b.canvas.height)/(a+e))),k=c/(a+e)|0,l=-c%(a+e),i,p=Math.min(this._rows[0].length,Math.ceil((c+b.canvas.width)/(a+e))),m,j,q,r=[];for(h=f;h<o;++h)if(0<=h){q=this._rows[h];for(i=k;i<p;++i)if(0<=i&&(m=q[i],j=m.getColor()))n=m.getScale(),_.isNumber(n)||(n=1),1!==n&&(r.push(m),j=m.getColor(!0),n=1),j&&(this.borderFill&&(b.fillStyle=this.borderFill,b.fillRect((i-k)*(a+e)+l,(h-f)*(a+e)+g,a+2*e,e),b.fillRect((i-
k)*(a+e)+l,(h-f)*(a+e)+g+a+e,a+2*e,e),b.fillRect((i-k)*(a+e)+l,(h-f)*(a+e)+g,e,a+2*e),b.fillRect((i-k)*(a+e)+l+a+e,(h-f)*(a+e)+g,e,a+2*e)),b.fillStyle=j,j=Math.round(a*n),n=a/2-j/2,b.fillRect((i-k)*(a+e)+e+n+l,(h-f)*(a+e)+e+n+g,j,j))}r.sort(function(a,c){return a.scale-c.scale});for(h=0;h<r.length;++h){m=r[h];var n=m.getScale();if(j=m.getColor())b.fillStyle=j,j=Math.round(a*n),n=a/2-j/2,b.fillRect((m.x-k)*(a+e)+e+n+l,(m.y-f)*(a+e)+e+n+g,j,j)}for(h=0;h<this.freeActors.length;++h)if(e=this.freeActors[h],
j=L7.Color.toCssString(e.color))b.fillStyle=j,b.fillRect(e.position.x-c,e.position.y-d,a,a)}}})();
...
if(this.isRgbString(c)){for(var d=c.indexOf("("),e=[],c=c.substring(d+1).split(","),d=0,f=c.length;d<f;++d)e.push(parseFloat(c[d]));3===e.length&&e.push(1);return e}},fromArrayToHex:function(a,b){for(var e="#",f=b&&b.includeAlpha&&4===a.length?4:3,g=0;g<f;++g){var h=a[g].toString(16);2>h.length&&(h="0"+h);e+=h}return e.toUpperCase()},composite:function(a){for(var b=this.toArray(arguments[0]).slice(0),e=1;e<arguments.length;++e){var f=b,g=this.toArray(arguments[e]),h=g[3],o=f[3],k=1-h,l=void 0,i=void 0;
for(l=0,i=f.length-1;l<i;++l)f[l]=Math.round(g[l]*h+f[l]*o*k);f[3]=(f[3]+g[3])/2}return b},fromFloats:function(a,b,e,f){return[Math.round(255*a),Math.round(255*b),Math.round(255*e),f]}};var b={aliceblue:"#F0F8FF",antiquewhite:"#FAEBD7",aqua:"#00FFFF",aquamarine:"#7FFFD4",azure:"#F0FFFF",beige:"#F5F5DC",bisque:"#FFE4C4",black:"#000000",blanchedalmond:"#FFEBCD",blue:"#0000FF",blueviolet:"#8A2BE2",brown:"#A52A2A",burlywood:"#DEB887",cadetblue:"#5F9EA0",chartreuse:"#7FFF00",chocolate:"#D2691E",coral:"#FF7F50",
...
this.viewport=new L7.Viewport(this);this.board&&(this.board.viewport=this.viewport);this.container=this.container||document.body;this.canvas=this._createCanvas();L7.Keys.init(this.canvas);L7.Mouse.init(this.canvas);this.delays=[];var c=this;L7.global.addEventListener("blur",function(){delete c._lastTimestamp})};L7.Game.prototype={_createCanvas:function(){var a=document.createElement("canvas");a.width=this.viewport.width;a.height=this.viewport.height;a.style.imageRendering="-webkit-optimize-contrast";
this.container.appendChild(a);return a},go:function(){delete this._lastTimestamp;this._doFrame(Date.now())},after:function(a,c){this.delays.add({remaining:a,handler:c})},replaceBoard:function(a){this.board&&this.board.destroy&&this.board.destroy();this.board=a;this.board.viewport=this.viewport;this.viewport.reset()},_updateDelays:function(a){var c=[];this.delays.forEach(function(d){d.remaining-=a;0>=d.remaining&&(d.handler.call(d.scope,this),c.push(d))},this);c.forEach(function(a){this.delays.remove(a)},
this)},_doFrame:function(b){var c=b-(this._lastTimestamp||b);this._lastTimestamp=b;for(var d=c;0<d;){var e=Math.min(a,d);this._updateDelays(e);this.board.update(e,b);d-=e}this.webgl?(d=this.canvas.getContext("experimental-webgl"),d.viewportWidth=this.canvas.width,d.viewportHeight=this.canvas.height):(d=this.canvas.getContext("2d"),d.clearRect(0,0,this.canvas.width,this.canvas.height));this.board[this.webgl?"rendergl":"render"](c,d,this.viewport.anchorX,this.viewport.anchorY,b);this.paused||requestAnimationFrame(this._doFrame)}};
Object.defineProperty(L7.Game.prototype,"paused",{get:function(){return this._paused},set:function(a){(this._paused=a)||this.go()}})})();
...
(function(){var a=Math.PI,b=2*a,c=a/2;L7.Pair=function(a,c){this._a=a||0;this._b=c||0};L7.Pair.prototype={clone:function(){return L7.p(this.x,this.y)},equals:function(a){return a?a===this||a._a===this._a&&a._b==this._b||a.x===this._a&&a.y==this._b||a.width==this._a&&a.height==this._b:!1},up:function(){return L7.p(this.x,this.y-1)},left:function(){return L7.p(this.x-1,this.y)},right:function(){return L7.p(this.x+1,this.y)},down:function(){return L7.p(this.x,this.y+1)},add:function(a,c){return L7.p(this.x+
("number"===typeof a.x?a.x:a||0),this.y+("number"===typeof a.y?a.y:c||0))},subtract:function(a,c){x="number"===typeof a.x?a.x:a||0;y="number"===typeof a.y?a.y:c||0;return L7.p(this.x-x,this.y-y)},negate:function(){return L7.p(-this.x,-this.y)},multiply:function(a){return L7.p(this.x*a,this.y*a)},round:function(){return L7.pr(this.x,this.y)},dot:function(a){return this.x*a.x+this.y*a.y},length:function(){return Math.sqrt(this.dot(this))},normalize:function(){return this.multiply(1/this.length())},
...
a.removeActor(b)},this);this._addedActors=!1},reset:function(){this._particleIndex=this._particleCount=0},_isFull:function(){return this._particleCount===this.totalParticles},_initParticle:function(c){c.rx=this.position.x+this.posVar.x*a();c.ry=this.position.y+this.posVar.y*a();var b=L7.degreesToRadians(this.angle+this.angleVar*a()),b=L7.p(Math.cos(b),Math.sin(b)),e=this.speed+this.speedVar*a(),b=b.multiply(e);c.dir={x:b.x,y:b.y};c.radialAccel=this.radialAccel+this.radialAccelVar*a();c.radialAccel&&
(c.radialAccel=0);c.tangentialAccel=this.tangentialAccel+this.tangentialAccelVar*a();c.tangentialAccel||(c.tangentialAccel=0);b=this.life+this.lifeVar*a();c.life=Math.max(0,b);b=[this.startColor[0]+this.startColorVar[0]*a(),this.startColor[1]+this.startColorVar[1]*a(),this.startColor[2]+this.startColorVar[2]*a(),this.startColor[3]+this.startColorVar[3]*a()];e=[this.endColor[0]*this.endColorVar[0]*a(),this.endColor[1]*this.endColorVar[1]*a(),this.endColor[2]*this.endColorVar[2]*a(),this.endColor[3]*
this.endColorVar[3]*a()];c.color=b;c.pieces[0].color=b;c.deltaColor=[(e[0]-b[0])/c.life,(e[1]-b[1])/c.life,(e[2]-b[2])/c.life,(e[3]-b[3])/c.life];_.isUndefined(this.startSize)?(c.size=1,c.deltaSize=0):(b=this.startSize+this.startSizeVar*a(),b=Math.max(0,b),c.size=b,_.isUndefined(this.endSize)?c.deltaSize=0:(e=this.endSize+this.endSizeVar*a(),c.deltaSize=(e-b)/c.life))},_addParticle:function(){if(this._isFull())return!1;this._initParticle(this._particles[this._particleCount]);++this._particleCount;
