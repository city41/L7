...
h,p=Math.min(this._rows.length,Math.ceil((d+b.canvas.height)/(a+e))),n=(0>c?Math.ceil:Math.floor)(c/(a+e)),j=-c%(a+e),k,r=Math.min(this._rows[0].length,Math.ceil((c+b.canvas.width)/(a+e))),m,i,q,o=[];for(h=f;h<p;++h)if(0<=h){q=this._rows[h];for(k=n;k<r;++k)if(0<=k&&(m=q[k],i=m.getColor()))l=m.getScale(),_.isNumber(l)||(l=1),1!==l&&(o.push(m),i=m.getColor(!0),l=1),i&&(this.borderFill&&(b.fillStyle=this.borderFill,b.fillRect((k-n)*(a+e)+j,(h-f)*(a+e)+g,a+2*e,e),b.fillRect((k-n)*(a+e)+j,(h-f)*(a+e)+
g+a+e,a+2*e,e),b.fillRect((k-n)*(a+e)+j,(h-f)*(a+e)+g,e,a+2*e),b.fillRect((k-n)*(a+e)+j+a+e,(h-f)*(a+e)+g,e,a+2*e)),b.fillStyle=i,i=Math.round(a*l),l=a/2-i/2,b.fillRect((k-n)*(a+e)+e+l+j,(h-f)*(a+e)+e+l+g,i,i))}o.sort(function(a,c){return a.scale-c.scale});for(h=0;h<o.length;++h){m=o[h];var l=m.getScale();if(i=m.getColor())b.fillStyle=i,i=Math.round(a*l),l=a/2-i/2,b.fillRect((m.x-n)*(a+e)+e+l+j,(m.y-f)*(a+e)+e+l+g,i,i)}for(h=0;h<this.freeActors.length;++h)if(e=this.freeActors[h],i=L7.Color.toCssString(e.color))b.fillStyle=
...
if(this.isRgbString(c)){for(var d=c.indexOf("("),e=[],c=c.substring(d+1).split(","),d=0,f=c.length;d<f;++d)e.push(parseFloat(c[d]));3===e.length&&e.push(1);return e}},fromArrayToHex:function(a,b){for(var e="#",f=b&&b.includeAlpha&&4===a.length?4:3,g=0;g<f;++g){var h=a[g].toString(16);2>h.length&&(h="0"+h);e+=h}return e.toUpperCase()},composite:function(a){for(var b=this.toArray(arguments[0]).slice(0),e=1;e<arguments.length;++e){var f=b,g=this.toArray(arguments[e]),h=g[3],p=f[3],n=1-h,j=void 0,k=void 0;
for(j=0,k=f.length-1;j<k;++j)f[j]=Math.round(g[j]*h+f[j]*p*n);f[3]=(f[3]+g[3])/2}return b},fromFloats:function(a,b,e,f){return[Math.round(255*a),Math.round(255*b),Math.round(255*e),f]}};var b={aliceblue:"#F0F8FF",antiquewhite:"#FAEBD7",aqua:"#00FFFF",aquamarine:"#7FFFD4",azure:"#F0FFFF",beige:"#F5F5DC",bisque:"#FFE4C4",black:"#000000",blanchedalmond:"#FFEBCD",blue:"#0000FF",blueviolet:"#8A2BE2",brown:"#A52A2A",burlywood:"#DEB887",cadetblue:"#5F9EA0",chartreuse:"#7FFF00",chocolate:"#D2691E",coral:"#FF7F50",
